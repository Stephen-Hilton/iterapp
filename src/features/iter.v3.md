...
- **Idle usage probe** (V2 limits.probe): engine `probe_stale_min` (default 20): when nothing is running and the active account's snapshot is older than that, one haiku nudge refreshes it (same path as the chip's test button).
